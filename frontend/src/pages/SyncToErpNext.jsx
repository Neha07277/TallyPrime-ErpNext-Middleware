...
const C = {
  card:"#ffffff",surface:"#f4f5f9",bg:"#eef0f6",border:"#e2e5ef",borderH:"#c8cedd",
  ink:"#0d1117",muted:"#6b7280",dim:"#9ca3af",
...
function Spinner({size=14,color=C.accent}){return <span style={{display:"inline-block",width:size,height:size,borderRadius:"50%",border:`2px solid ${color}25`,borderTopColor:color,animation:"se-spin .7s linear infinite",flexShrink:0}}/>;}
const inp=(extra={})=>({width:"100%",padding:"9px 13px",border:`1.5px solid ${C.border}`,borderRadius:10,fontFamily:C.sans,fontSize:13,color:C.ink,background:C.surface,outline:"none",transition:"border-color .15s,background .15s,box-shadow .15s",boxSizing:"border-box",...extra});
...
function StatusBadge({status}){
  ...
  return <span style={{fontFamily:C.mono,fontSize:9,fontWeight:700,letterSpacing:"0.12em",padding:"3px 9px",borderRadius:20,background:s.bg,border:`1.5px solid ${s.bd}`,color:s.color}}>{s.label}</span>;
}
function StatPill({label,value,color="blue"}){
  ...
  const s=map[color];
  return <span style={{fontFamily:C.mono,fontSize:10,fontWeight:600,background:s.bg,border:`1.5px solid ${s.bd}`,color:s.color,borderRadius:8,padding:"3px 9px"}}>{label}: <strong>{value}</strong></span>;
}
function StepResult({title,data}){
  if(!data)return null;
  const isOk=data.status==="ok"||data.status==="warn";
  return(
    <div style={{background:isOk?C.greenL:C.redL,border:`1.5px solid ${isOk?C.greenB:C.redB}`,borderRadius:12,padding:"12px 16px",display:"flex",flexDirection:"column",gap:8}}>
...
function JobProgressBanner({jobId,type}){
  ...
  return(
    <div style={{background:C.accentL,border:`1.5px solid ${C.accentB}`,borderRadius:14,padding:"16px 20px",display:"flex",alignItems:"center",gap:16,animation:"se-fade .2s ease"}}>
      <div style={{width:42,height:42,borderRadius:12,flexShrink:0,background:C.card,border:`1.5px solid ${C.accentB}`,display:"flex",alignItems:"center",justifyContent:"center"}}><Spinner size={18}/></div>
      <div style={{flex:1}}>
...
function ErpCredentialsPanel({company,onSaved}){
  ...
  return(
    <div style={{background:C.card,border:`1.5px solid ${hasCreds?C.greenB:C.amberB}`,borderRadius:12,overflow:"hidden",marginBottom:16}}>
      <button onClick={()=>setShow(s=>!s)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"13px 16px",background:"none",border:"none",cursor:"pointer",textAlign:"left"}}>
        <div style={{width:34,height:34,borderRadius:9,flexShrink:0,background:hasCreds?C.greenL:C.amberL,border:`1.5px solid ${hasCreds?C.greenB:C.amberB}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{hasCreds?"🔐":"⚙️"}</div>
        <div style={{flex:1}}>
        ...
        <span style={{fontFamily:C.mono,fontSize:11,color:C.muted,transform:show?"rotate(90deg)":"none",transition:"transform .2s",flexShrink:0}}>▶</span>
      </button>
      {show&&(
        <div style={{padding:"0 16px 16px",display:"flex",flexDirection:"column",gap:12,borderTop:`1px solid ${C.border}`,background:C.surface}}>
          <p style={{fontFamily:C.mono,fontSize:10,color:C.muted,paddingTop:12,margin:0,lineHeight:1.6}}>Credentials are saved locally per Tally company and override .env values for this session.</p>
          <div>
            <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:5,fontWeight:600}}>ERPNext URL</label>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {[["API Key",key,setKey,"api_key","text"],["API Secret",secret,setSecret,"api_secret","password"]].map(([lbl,val,setter,ph,type])=>(
              <div key={lbl}>
                <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:5,fontWeight:600}}>{lbl}</label>
...
function SectionHead({step,title,done}){
  return(
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18,paddingBottom:14,borderBottom:`1px solid ${C.border}`}}>
      <div style={{width:26,height:26,borderRadius:8,flexShrink:0,background:done?C.green:`linear-gradient(135deg,${C.accent},${C.accentD})`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:done?`0 2px 8px ${C.green}44`:`0 2px 8px ${C.accent}44`}}>
        <span style={{fontFamily:C.mono,fontSize:10,fontWeight:800,color:"#fff"}}>{done?"✓":step}</span>
      </div>
      <h2 style={{fontFamily:C.title,fontWeight:800,fontSize:14,color:C.ink,flex:1,letterSpacing:"-0.3px",margin:0}}>{title}</h2>
    </div>
  );
}
...
export function SyncToErpNext({companies}){
  ...
  useEffect(()=>{
    ...
    try{const job=JSON.parse(stored);if(!job?.jobId||!job?.type)return;
      fetch(`${BASE_URL}/sync/status/${job.jobId}`).then(r=>r.json()).then(data=>{
    ...
    return new Promise(resolve=>{
      function tick(){
        ...
        pollRef.current=setTimeout(async()=>{
          try{const res=await fetch(`${BASE_URL}/sync/status/${jobId}`);const data=await res.json();const job=data.job;
            if(!job){resolve({type,error:"Job not found on server"});return;}
  ...
  const runSync=useCallback(async(type)=>{
    ...
    try{
      ...
      if(type==="full"){
        // FIX: pass the actual checkbox flags so the backend runs only what was selected.
        // The old code sent wrong keys (includeVouchers/includeMasters) so ALL flags
        // arrived as false at the backend and nothing was ever synced.
  ...
  const card={background:C.card,border:`1.5px solid ${C.border}`,borderRadius:16,padding:22,boxShadow:"0 1px 4px rgba(0,0,0,.05)"};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:16,fontFamily:C.sans}}>
      <style>{`
        ...
        @keyframes se-pop{0%{transform:scale(.95);opacity:0}100%{transform:scale(1);opacity:1}}
        @keyframes se-pulse{0%,100%{opacity:1}50%{opacity:.35}}
        .se-opt:hover{box-shadow:0 2px 10px rgba(0,0,0,.08)!important}
        .se-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 8px 24px rgba(37,99,235,.4)!important}
        .se-btn:active:not(:disabled){transform:translateY(0)}
      `}</style>

      {/* Floating stop */}
      {(loading||activeJob)&&(
        <div style={{position:"fixed",bottom:28,right:28,zIndex:9999,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:10,animation:"se-pop .2s ease"}}>
          <div style={{background:C.card,border:`1.5px solid ${C.accentB}`,borderRadius:14,padding:"11px 18px",display:"flex",alignItems:"center",gap:10,boxShadow:"0 8px 32px rgba(0,0,0,.14)"}}>
            <Spinner size={13}/><span style={{fontFamily:C.mono,fontSize:11,color:C.muted}}>{activeJob?`Syncing ${activeJob.type}…`:`Syncing ${loading}…`}</span>
          </div>
          <button
            ...
            disabled={cancelling}
            style={{display:"flex",alignItems:"center",gap:8,padding:"13px 24px",borderRadius:12,border:"none",background:cancelling?C.surface:C.red,color:cancelling?C.muted:"#fff",fontFamily:C.title,fontSize:13,fontWeight:800,cursor:cancelling?"not-allowed":"pointer",boxShadow:cancelling?"none":"0 6px 24px rgba(220,38,38,.4)",transition:"all .15s"}}
          >
            {cancelling?<Spinner size={13} color={C.muted}/>:<span>■</span>}{cancelling?"Stopping…":"Stop Sync"}
          </button>
        </div>
      )}
      ...
      <div style={card}>
        ...
        <div style={{marginBottom:16}}>
          <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:6,fontWeight:600}}>ERPNext Company Name <span style={{color:C.amber}}>— must match exactly</span></label>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <button onClick={async()=>{setPinging(true);setErpPing(null);try{const creds=credsOverride();if(creds.erpnextUrl){const res=await fetch(`${BASE_URL}/erpnext/ping`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(creds)});let _d;try{_d=await res.json();}catch(_){_d={connected:false,error:"Backend returned non-JSON (status "+res.status+")"};} setErpPing(_d);}else{setErpPing(await tallyAPI.erpnextPing());}}catch(e){const m=e.message||"Unknown";setErpPing({connected:false,error:m.includes("fetch")?`Cannot reach backend at ${BASE_URL} — is the server running on port 4000?`:m});}finally{setPinging(false);}}} disabled={pinging}
            style={{display:"flex",alignItems:"center",gap:7,padding:"9px 18px",borderRadius:9,background:C.accentL,border:`1.5px solid ${C.accentB}`,color:C.accentD,fontFamily:C.mono,fontSize:11,fontWeight:600,cursor:pinging?"not-allowed":"pointer",opacity:pinging?0.7:1,transition:"all .15s",flexShrink:0}}>
            {pinging&&<Spinner size={11}/>}{pinging?"Testing…":"🔌 Test Connection"}
          </button>
          {erpPing&&(
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderRadius:9,flex:1,background:erpPing.connected?C.greenL:C.redL,border:`1.5px solid ${erpPing.connected?C.greenB:C.redB}`,animation:"se-fade .2s ease"}}>
              <span style={{fontSize:14}}>{erpPing.connected?"✓":"✗"}</span>
      ...
      <div style={card}>
        ...
        <div style={{marginBottom:14}}>
          <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:6,fontWeight:600}}>Tally Company</label>
          {companies?.length>0?(
            <select value={company} onChange={e=>setCompany(e.target.value)} style={{...inp(),cursor:"pointer",appearance:"none"}} onFocus={onFocus} onBlur={onBlur}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[["From Date",fromDate,setFromDate],["To Date",toDate,setToDate]].map(([lbl,v,setter])=>(
            <div key={lbl}>
              <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:6,fontWeight:600}}>{lbl}</label>
        ...
        {dayCount>90&&(
          <div style={{marginTop:12,padding:"10px 14px",borderRadius:9,background:C.amberL,border:`1.5px solid ${C.amberB}`,display:"flex",gap:9,alignItems:"flex-start"}}>
            <span style={{fontSize:14,flexShrink:0}}>⏱</span>
            <p style={{fontFamily:C.mono,fontSize:10,color:C.amber,margin:0,lineHeight:1.6}}><strong>Large date range ({dayCount} days)</strong> — Voucher sync will be split into <strong>~{chunkCount} chunks of 15 days</strong> to prevent Tally from hanging. This is normal and safe.</p>
          </div>
        )}
      </div>
      ...
      <div style={card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:22}}>
          {SYNC_OPTIONS.map(opt=>(
            <label key={opt.key} className="se-opt" style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"11px 14px",borderRadius:11,background:syncOpts[opt.key]?C.accentL:C.surface,border:`1.5px solid ${syncOpts[opt.key]?C.accentB:C.border}`,transition:"all .15s",userSelect:"none",boxShadow:syncOpts[opt.key]?`0 2px 8px ${C.accent}22`:"none"}}>
              ...
              <div style={{width:32,height:32,borderRadius:8,flexShrink:0,background:syncOpts[opt.key]?C.card:`${C.border}80`,border:`1px solid ${syncOpts[opt.key]?C.accentB:C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,transition:"all .15s"}}>{opt.icon}</div>
              <div style={{minWidth:0,flex:1}}>
                <p style={{fontFamily:C.title,fontSize:12,fontWeight:700,color:syncOpts[opt.key]?C.accentD:C.ink,margin:0,letterSpacing:"-0.2px"}}>{opt.label}</p>
              ...
              <button onClick={e=>{e.preventDefault();e.stopPropagation();runSync(opt.individual);}} disabled={busy||!co} title={`Sync ${opt.label} only`}
                style={{padding:"4px 10px",borderRadius:6,border:`1.5px solid ${C.accentB}`,background:C.card,color:C.accentD,fontFamily:C.mono,fontSize:9,fontWeight:600,cursor:busy||!co?"not-allowed":"pointer",flexShrink:0,opacity:busy||!co?0.4:1,transition:"all .15s"}}>
                {loading===opt.individual?<Spinner size={9}/>:"↑ Sync"}
              </button>
            </label>
          ))}
        </div>
        <button className="se-btn" onClick={()=>runSync("full")} disabled={busy||!co||noSync||!erpCompany}
          style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:12,padding:"16px 24px",borderRadius:12,border:"none",background:busy||!isReady?C.surface:`linear-gradient(135deg,${C.accent},${C.accentD})`,color:busy||!isReady?C.dim:"#fff",fontFamily:C.title,fontSize:15,fontWeight:800,letterSpacing:"-0.3px",cursor:busy||!isReady?"not-allowed":"pointer",boxShadow:busy||!isReady?"none":`0 4px 20px ${C.accent}44`,transition:"all .2s"}}>
          {loading==="full"?<><Spinner color={busy&&isReady?"#fff":C.dim} size={16}/> Syncing to ERPNext…</>:<><span style={{fontSize:18}}>⇄</span> Sync Selected → ERPNext</>}
          {!busy&&isReady&&<span style={{fontFamily:C.mono,fontSize:10,opacity:0.65,fontWeight:400}}>{Object.values(syncOpts).filter(Boolean).length} item{Object.values(syncOpts).filter(Boolean).length!==1?"s":""} selected</span>}
        </button>
        {!isReady&&!busy&&(
          <div style={{marginTop:10,padding:"10px 14px",borderRadius:9,background:C.amberL,border:`1.5px solid ${C.amberB}`}}>
      ...
      <div style={card}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:autoMode?20:0}}>
          {[{id:false,icon:"🖱",label:"Manual",desc:"Sync on demand"},{id:true,icon:"⏱",label:"Auto Sync",desc:"Runs on schedule"}].map(m=>(
            <button key={String(m.id)} onClick={()=>{setAutoMode(m.id);if(!m.id)setAutoRunning(false);}}
              style={{flex:1,padding:"13px 16px",borderRadius:11,border:`1.5px solid ${autoMode===m.id?C.accentD:C.border}`,background:autoMode===m.id?`linear-gradient(135deg,${C.accent},${C.accentD})`:C.surface,cursor:"pointer",transition:"all .15s",display:"flex",flexDirection:"column",alignItems:"center",gap:5,boxShadow:autoMode===m.id?`0 4px 16px ${C.accent}33`:"none"}}>
              <span style={{fontSize:22}}>{m.icon}</span>
              <span style={{fontFamily:C.title,fontSize:13,fontWeight:700,color:autoMode===m.id?"#fff":C.ink}}>{m.label}</span>
              <span style={{fontFamily:C.mono,fontSize:9,color:autoMode===m.id?"rgba(255,255,255,.6)":C.muted}}>{m.desc}</span>
            </button>
          ))}
        </div>
        {autoMode&&(
          <div style={{animation:"se-fade .2s ease"}}>
            <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:8,fontWeight:600}}>Sync Interval</label>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
              {INTERVALS.map(iv=>(
                <button key={iv.value} onClick={()=>setAutoInterval(iv.value)} disabled={autoRunning}
                  style={{padding:"5px 13px",borderRadius:20,background:autoInterval===iv.value?C.accentD:C.surface,border:`1.5px solid ${autoInterval===iv.value?C.accentD:C.border}`,color:autoInterval===iv.value?"#fff":C.muted,fontFamily:C.mono,fontSize:10,fontWeight:600,cursor:autoRunning?"not-allowed":"pointer",opacity:autoRunning&&autoInterval!==iv.value?0.4:1,transition:"all .15s"}}>
                  {iv.label}
                </button>
              ))}
            </div>
            <div style={{display:"flex",alignItems:"center",gap:14}}>
              <button onClick={()=>setAutoRunning(r=>!r)} disabled={!co||noSync}
                style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:9,padding:"12px 18px",borderRadius:10,border:"none",background:autoRunning?C.red:C.green,color:"#fff",fontFamily:C.title,fontSize:13,fontWeight:700,cursor:!co||noSync?"not-allowed":"pointer",opacity:!co||noSync?0.5:1,transition:"all .15s",boxShadow:autoRunning?"0 4px 16px rgba(220,38,38,.3)":"0 4px 16px rgba(22,163,74,.3)"}}>
                {autoRunning?<><span>■</span> Stop Auto-Sync</>:<><span>▶</span> Start Auto-Sync</>}
              </button>
              {autoRunning&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}><CountdownRing remainingMs={autoRemainingMs} totalMs={autoInterval}/><span style={{fontFamily:C.mono,fontSize:9,color:C.muted}}>next run</span></div>}
            </div>
            {autoRunning&&(
              <div style={{marginTop:14,padding:"12px 16px",borderRadius:10,background:C.accentL,border:`1.5px solid ${C.accentB}`,display:"flex",alignItems:"center",gap:10}}>
                <span style={{width:8,height:8,borderRadius:"50%",background:C.accent,flexShrink:0,animation:"se-pulse 1.4s ease-in-out infinite"}}/>
                <div style={{flex:1}}>
                ...
                {autoSyncing&&<Spinner size={13}/>}
              </div>
            )}
            {autoHistory.length>0&&(
              <div style={{marginTop:16}}>
                <label style={{display:"block",fontFamily:C.mono,fontSize:9,color:C.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:8,fontWeight:600}}>Sync History</label>
                <div style={{display:"flex",flexDirection:"column",gap:4}}>
                  {autoHistory.map((h,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 13px",borderRadius:9,background:C.surface,border:`1.5px solid ${C.border}`}}>
                      <span style={{width:8,height:8,borderRadius:"50%",flexShrink:0,background:h.status==="ok"?C.green:h.status==="failed"?C.red:C.amber}}/>
      ...
      {!autoMode&&result&&!activeJob&&(
        <div style={{animation:"se-pop .25s ease"}}>
          {result.error?(
            <div style={{background:C.redL,border:`1.5px solid ${C.redB}`,borderRadius:14,padding:"16px 20px"}}>
            ...
            <div style={{background:C.card,border:`1.5px solid ${C.greenB}`,borderRadius:16,padding:20,display:"flex",flexDirection:"column",gap:12,boxShadow:`0 4px 20px ${C.green}18`}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:14,borderBottom:`1px solid ${C.border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:32,height:32,borderRadius:9,background:C.greenL,border:`1.5px solid ${C.greenB}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>✓</div>
                  <span style={{fontFamily:C.title,fontSize:14,fontWeight:800,color:C.ink,letterSpacing:"-0.3px"}}>Sync Complete</span>
                </div>
