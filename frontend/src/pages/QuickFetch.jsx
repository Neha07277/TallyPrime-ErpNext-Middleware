...
function FetchCard({ action, results, loading }) {
  ...
  return (
    <div style={{
      background: C.card,
      border:`1.5px solid ${hasData ? C.greenB : hasError ? "#fca5a5" : hov && !disabled ? C.borderH : C.border}`,
      borderRadius:14,
      overflow:"hidden",
      ...
      >
        ...
        {busy
          ...
          : hasError
          ? <span style={{
              fontFamily:C.mono, fontSize:18, color:"#fca5a5",
            }}>✗</span>
          : <span style={{
...
export function QuickFetch({ companies }) {
  ...
  // ── Sections (unchanged logic, enriched with enabled flag) ────────────────
  const sections = [
    {
      label: null,
      items: [
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:6, fontFamily:C.sans }}>
      ...
      <div style={{
        ...
        borderRadius:16, padding:22, marginBottom:8,
        ...
        {!co && (
          <div style={{
            marginTop:14, padding:"9px 13px", borderRadius:9,
      ...
      {sections.map((section) => (
        <div key={section.label || "top"} style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
